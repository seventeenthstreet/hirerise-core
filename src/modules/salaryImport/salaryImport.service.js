...
const { validateSalaryRecord, logImport }         = require('../salary/salary.service');
...
async function resolveRoleId(roleName, roleCache) {
  ...
  if (db) {
    const snap = await db.collection('cms_roles')
...
async function importSalariesFromCSV(buffer, adminId, ipAddress = null) {
  ...
  logger.info('[SalaryImport] CSV parsed', { rowCount: rows.length, adminId });
  ...
  for (const row of rows) {
    ...
    if (!row.role) {
      errors.push({ row: rowNum, field: 'role', message: 'role is required' });
      skipped++;
      continue;
    }
    ...
    if (!roleId) {
      errors.push({ row: rowNum, field: 'role', message: `Role not found: "${row.role}"` });
      skipped++;
      continue;
    }

    if (row.minSalary === null || row.medianSalary === null || row.maxSalary === null) {
      errors.push({ row: rowNum, field: 'salary', message: 'minSalary, medianSalary, maxSalary must be numeric' });
      skipped++;
      continue;
    }
    ...
    try {
      ...
      errors.push({ row: rowNum, field: 'salary', message: err.message });
      skipped++;
      continue;
    }
  ...
  // ── 5. Batch insert ───────────────────────────────────────────────────────
  const { inserted, duplicates } = await salaryRepository.batchInsert(validRecords, adminId);
...
module.exports = { importSalariesFromCSV };
