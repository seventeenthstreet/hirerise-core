...
async function extractTextFromBuffer(buffer, mimetype, originalname) {
  ...
  if (mimetype === 'application/pdf' || ext === '.pdf') {
    try {
      ...
      throw new AppError('Could not extract text from PDF. Please ensure it is not scanned/image-only.', 422, {
        originalname
  ...
  throw new AppError(`Unsupported file type: ${ext || mimetype}. Please upload a PDF, DOCX, DOC, or TXT file.`, 415, {
    mimetype,
    ext
  }, ErrorCodes.VALIDATION_ERROR);
}
...
function primaryCvCheck(resumeText) {
  ...
  if (wordCount < 80) {
    return {
      pass: false,
      reason: 'Document is too short to be a CV (under 80 words).'
    };
  }

  // Must contain at least 2 of these CV signal words
  const cvSignals = ['experience', 'education', 'skills', 'qualification', 'employment', 'university', 'college', 'degree', 'diploma', 'certification', 'work', 'job', 'position', 'role', 'company', 'organisation', 'projects', 'achievements', 'responsibilities', 'objective', 'summary', 'b.com', 'm.com', 'bba', 'mba', 'bsc', 'msc', 'b.tech', 'm.tech'];
  const matchCount = cvSignals.filter(s => text.includes(s)).length;
  if (matchCount < 2) {
    return {
      pass: false,
      reason: 'Document does not contain enough professional CV keywords.'
    };
  }

  // Reject obvious non-CV patterns
  const rejectPatterns = [/invoice\s*(no|number|#)/i, /bill\s*to/i, /total\s*amount/i, /terms\s*and\s*conditions/i, /chapter\s+\d+/i, /abstract:/i, /dear\s+(sir|madam|mr|ms)/i];
  for (const pattern of rejectPatterns) {
    if (pattern.test(resumeText)) {
      return {
        pass: false,
        reason: 'Document matches a non-CV pattern (invoice, letter, or academic paper).'
      };
    }
  }
  return {
...
async function validateIsResume(resumeText, originalname) {
  ...
  if (!preCheck.pass) {
    ...
    throw new AppError(`The uploaded document doesn't appear to be a CV or resume. ${preCheck.reason}`, 422, {
      originalname,
      reason: preCheck.reason
    }, ErrorCodes.VALIDATION_ERROR);
  }
  logger.debug('[ResumeService] CV pre-check passed — sending to Claude', {
    originalname
  });
  ...
  if (!validation.isValidCV && validation.confidenceScore >= 40) {
    throw new AppError(`The uploaded document doesn't appear to be a CV or resume. ` + `Please upload your resume to continue. (${validation.reason})`, 422, {
      originalname,
      reason: validation.reason,
      category: validation.category
    }, ErrorCodes.VALIDATION_ERROR);
...
async function uploadToStorage(buffer, storagePath, mimetype) {
  ...
  const {
    data,
    error: urlError
  } = await supabase.storage.from(BUCKET).createSignedUrl(storagePath, URL_TTL_SECONDS);
...
async function uploadResume(userId, file, options = {}) {
  ...
  if (!file || !file.buffer) {
    throw new AppError('No file uploaded. Please attach a resume file.', 400, {}, ErrorCodes.VALIDATION_ERROR);
  }
  if (!userId) {
  ...
  if (size > MAX_BYTES) {
    throw new AppError(`File too large. Maximum allowed size is ${Math.round(MAX_BYTES / 1024 / 1024)} MB.`, 413, {
      size,
      maxBytes: MAX_BYTES
    }, ErrorCodes.VALIDATION_ERROR);
  }
  logger.debug('[ResumeService] uploadResume start', {
    userId,
    originalname,
    size
  });
  ...
  try {
    ...
    logger.error('[ResumeService] Text extraction failed', {
      error: err.message
    });
    throw new AppError('Failed to read resume file. The file may be corrupted or password-protected.', 422, {
      originalname
    }, ErrorCodes.VALIDATION_ERROR);
  }
  if (!resumeText || resumeText.trim().length < 50) {
    throw new AppError('Could not extract enough text from the uploaded file. ' + 'Please ensure the resume is not scanned/image-only.', 422, {
      originalname
    }, ErrorCodes.VALIDATION_ERROR);
  }
  ...
  try {
    ...
    logger.warn('[ResumeService] Local parser failed (non-fatal)', {
  ...
  if (process.env.NODE_ENV !== 'test') {
    try {
      ...
      logger.error('[ResumeService] Storage upload failed', {
        error: err.message
      });
      throw new AppError('File upload to storage failed. Please try again.', 502, {
        originalname
  ...
  const resumeDoc = {
    ...
    storagePath,
    signedUrlExpiresAt,
    // FIX G-03: null in test, Date in production
    mimetype,
    sizeBytes: size,
  ...
  // MIGRATION: batch → individual supabase calls

  // 2-4. Build skills array for profile sync (READ by jobMatchingEngine._loadUserProfile)
  //    Syncs users, userProfiles, and onboardingProgress in one parallel batch below.
  const skillsForProfile = (parsedResumeData?.skills ?? []).map(name => ({
    name,
    proficiency: 'intermediate'
  }));

  // Execute writes in parallel (replaces batch.commit())
  await Promise.all([supabase.from('resumes').upsert({
    id: resumeId,
    ...resumeDoc
  }, {
    onConflict: 'id'
  ...
  }).eq('id', userId), supabase.from('userProfiles').upsert({
    id: userId,
  ...
  try {
    ...
    } = require('../jobSeeker/jobMatchingEngine.service');
    await invalidateUserMatchCache(userId);
  } catch (cacheErr) {
    logger.warn('[ResumeService] Job match cache invalidation failed (non-fatal)', {
      error: cacheErr.message
    });
  }
  logger.debug('[ResumeService] uploadResume complete', {
    userId,
    resumeId
  });
  ...
  try {
    ...
    logger.info('[ResumeService] uploadResume — AI job enqueued', {
      userId,
      resumeId,
      jobId
    });
...
async function scoreResume(userId, resumeId) {
  ...
  logger.debug('[ResumeService] scoreResume start', {
    userId,
    resumeId
  });
  ...
  const {
    data: _resumeRow,
    error: _resumeErr
  } = await supabase.from('resumes').select('*').eq('id', resumeId).maybeSingle();
  if (_resumeErr || !_resumeRow) {
    throw new AppError(`Resume '${resumeId}' not found`, 404, {
      resumeId
    }, ErrorCodes.NOT_FOUND);
  }
  const resumeData = _resumeRow;
  ...
  if (!resumeText || resumeText.trim().length < 50) {
    throw new AppError('Resume text is too short or missing. Please upload a valid resume.', 422, {
      resumeId
    }, ErrorCodes.VALIDATION_ERROR);
  }
  // ── Build prompt ──────────────────────────────────────────
  const userPrompt = [resumeData.targetRole ? `Target Role: ${resumeData.targetRole}` : null, resumeData.skillsDetected?.length ? `Detected Skills: ${resumeData.skillsDetected.join(', ')}` : null, `\nResume Text:\n${resumeText}`].filter(Boolean).join('\n');
  ...
  try {
    ...
    throw new AppError('Resume scoring failed. Please try again.', 502, {
      resumeId
    }, ErrorCodes.EXTERNAL_SERVICE_ERROR);
  }
  ...
  try {
    ...
    if (_scoreErr) logger.warn('[ResumeService] Failed to persist score', {
      resumeId,
      error: _scoreErr.message
    });
  } catch (err) {
...
async function analyzeResumeGrowth(userId, payload) {
  ...
  logger.debug('[ResumeService] analyzeResumeGrowth start', {
    userId,
    resumeId,
    targetRole
  });
  ...
  if (!_growthResume) throw new AppError(`Resume '${resumeId}' not found`, 404, {
    resumeId
  }, ErrorCodes.NOT_FOUND);
  const resumeData = _growthResume;
  if (resumeData.userId !== userId) throw new AppError('Unauthorized access to resume', 403, {
    resumeId
  }, ErrorCodes.UNAUTHORIZED);
  const resumeText = resumeData.resumeText;
  if (!resumeText || resumeText.trim().length < 50) throw new AppError('Resume text is too short or missing. Please upload a valid resume.', 422, {
    resumeId
  }, ErrorCodes.VALIDATION_ERROR);
...
${resumeText.trim()}`;
  ...
  try {
    ...
    throw new AppError('Resume growth analysis failed. Please try again.', 502, {
      resumeId
    }, ErrorCodes.EXTERNAL_SERVICE_ERROR);
  }
  ...
  try {
    await supabase.from('resume_growth_signals').upsert({
      id: signalId,
      ...signalDoc
    }, {
      onConflict: 'id'
...
async function refreshSignedUrl(userId, resumeId) {
  ...
  if (!_rSnap) throw new AppError(`Resume '${resumeId}' not found`, 404, {
    resumeId
  }, ErrorCodes.NOT_FOUND);
  const resumeData = _rSnap;
  if (resumeData.userId !== userId) throw new AppError('Unauthorized access to resume', 403, {
    resumeId
  }, ErrorCodes.UNAUTHORIZED);
  if (!resumeData.storagePath) {
    throw new AppError('Resume has no storage path — cannot refresh URL.', 422, {
      resumeId
    }, ErrorCodes.VALIDATION_ERROR);
  }
  ...
  if (stillValid) {
    logger.debug('[ResumeService] refreshSignedUrl — URL still valid, skipping', {
      resumeId
    });
    return {
      resumeId,
  ...
  try {
    ...
    const {
      data,
      error: urlError
    } = await supabase.storage.from(BUCKET).createSignedUrl(resumeData.storagePath, URL_TTL_SECONDS);
    if (urlError) throw new Error(urlError.message);
    newFileUrl = data.signedUrl;
  } catch (err) {
    logger.error('[ResumeService] refreshSignedUrl — storage error', {
      resumeId,
      error: err.message
    });
    throw new AppError('Failed to refresh CV URL. Please try again.', 502, {
      resumeId
    }, ErrorCodes.EXTERNAL_SERVICE_ERROR);
  }
  ...
  logger.info('[ResumeService] refreshSignedUrl — URL refreshed', {
    resumeId,
    newExpiresAt
  });
  return {
...
async function listResumes(userId) {
  ...
  const {
    data: _rDocs,
    error: _rErr
  } = await supabase.from('resumes').select('*').eq('userId', userId).eq('softDeleted', false).order('createdAt', {
    ascending: false
  }).limit(50);
  if (_rErr) logger.error('[ResumeService] listResumes error:', _rErr.message);
  const items = (_rDocs || []).map(d => {
    const doc = {
      id: d.id
    };
    return {
...
async function getResume(userId, resumeId) {
  ...
  if (!_rd) throw new AppError(`Resume '${resumeId}' not found`, 404, {
    resumeId
  }, ErrorCodes.NOT_FOUND);
  const d = _rd;
  if (d.userId !== userId) throw new AppError('Unauthorized access to resume', 403, {
    resumeId
  }, ErrorCodes.UNAUTHORIZED);
  return {
    id: snap.id,
...
async function deleteResume(userId, resumeId) {
  ...
  logger.info('[ResumeService] deleteResume — soft deleted', {
    userId,
    resumeId
  });
}
