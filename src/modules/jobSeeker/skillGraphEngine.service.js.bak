...
async function _loadUserProfile(userId) {
  ...
  return {
    ...
    industry:
      profile.industry || user.industry || progress.industry || null,
    yearsExperience:
...
async function getUserSkillGraph(userId) {
  ...
  return _cached(cacheKey, CACHE_TTL_SECONDS, async () => {
    const { skills, targetRole, industry } = await _loadUserProfile(userId);
...
async function detectSkillGap(userId) {
  ...
  return _cached(cacheKey, CACHE_TTL_SECONDS, async () => {
    const { skills, targetRole, industry, yearsExperience } = await _loadUserProfile(userId);
    ...
    if (gapResult && (gapResult.missing_required.length > 0 || gapResult.missing_preferred.length > 0)) {
      ...
      const missingRequired = (gapResult.missing_required || []).map(e => ({
        ...
        skill_id:     e.skill_id,
        demand_score: e.skill?.demand_score || 70,  // normalise to 0-100 scale for frontend
    ...
    for (const missing of missingHighDemand.slice(0, 3)) {
      try {
        const skillKey = missing.skill_id ||
          missing.name.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
        const path = await svc.generateLearningPath(skillKey, skills);
        if (path) learningPaths.push({ skill: missing.name, path });
      } catch (_) { /* non-fatal */ }
    }
    ...
    return {
      ...
      role_gap: gapResult ? {
        ...
        missing_required: (gapResult.missing_required || []).slice(0, 8)
          .map(e => e.skill?.skill_name || e.skill_id?.replace(/_/g,' ') || e.skill_id)
          .filter(Boolean),
        priority_missing: (gapResult.priority_missing || []).slice(0, 5)
          .map(e => ({
            name:     e.skill?.skill_name || e.skill_id?.replace(/_/g,' ') || e.skill_id,
...
module.exports = { getUserSkillGraph, detectSkillGap };
