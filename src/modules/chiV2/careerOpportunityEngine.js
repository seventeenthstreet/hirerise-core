...
const NORM_CEILINGS = Object.freeze({
  job_postings: 50_000,   // 50k postings = saturated / maximum demand
  growth_rate:  50,        // 50% YoY growth = extremely fast-growing
...
async function findReachableRoles(currentRoleId) {
  ...
  const queue     = [{ id: currentRoleId, steps: 0, years: 0 }];
  ...
  while (queue.length > 0) {
    ...
    for (const doc of snap.docs) {
      ...
      const nextYears = years + (Number(data.years_required) || 0);
 ...
 * Queries in batches of 10 (Firestore `in` limit).
...
async function fetchMarketDemand(roleIds, country = null) {
  ...
  const snaps = await Promise.all(
    chunks.map(chunk =>
      db.collection('role_market_demand')
  ...
  snaps.forEach(snap => {
    snap.docs.forEach(doc => {
      const data = doc.data();
      if (!demandMap.has(data.role_id)) demandMap.set(data.role_id, []);
      demandMap.get(data.role_id).push(data);
    });
  });
  ...
  if (country) {
    for (const [roleId, records] of demandMap.entries()) {
      const countryMatch = records.filter(
        r => String(r.country).toLowerCase() === country.toLowerCase()
      );
...
async function fetchRolesMeta(roleIds) {
  ...
  const meta  = {};
  const docs  = await Promise.all(roleIds.map(id => db.collection('roles').doc(id).get()));

  docs.forEach((doc, i) => {
    meta[roleIds[i]] = doc.exists
      ? { id: doc.id, ...doc.data() }
...
async function getMarketIntelligenceSummary(country = null) {
  // Fetch all market demand records (optionally filtered by country)
  let query = db.collection('role_market_demand');
  ...
  if (roleIds.length > 0) {
    ...
    const salarySnaps = await Promise.all(
      salaryChunks.map(chunk =>
        db.collection('role_salary_market').where('role_id', 'in', chunk).get()
      )
    );
    salarySnaps.forEach(s => {
      s.docs.forEach(d => {
        const data = d.data();
        if (!salaryMap[data.role_id] || (data.median_salary || 0) > (salaryMap[data.role_id] || 0)) {
          salaryMap[data.role_id] = data.median_salary;
        }
      });
    });
  }
 ...
 * @param {string}      profile.current_role_id  — resolved Firestore doc ID
...
async function analyseCareerOpportunities(profile, options = {}) {
  ...
  if (!current_role_id) {
    return {
      ...
      meta:                 { engine_version: 'opportunity_v1', total_candidates: 0 },
    };
  }
