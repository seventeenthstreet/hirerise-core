 ...
 *
 * Reads from:  lmi_job_market_data (Firestore)
 * Writes to:   lmi_skill_demand, lmi_career_market_scores (Firestore)
 *
...
const { COLLECTIONS, buildSkillDemandDoc, buildCareerScoreDoc } = require('../models/jobMarket.model');
const { aggregateSkillCounts } = require('./skillExtraction.service');
 ...
 *   1. Load all job market docs from Firestore
...
async function runFullAnalysis() {
  ...
  // ── Load job data ─────────────────────────────────────────────────────
  const snap = await db.collection(COLLECTIONS.JOB_MARKET).get();
  if (snap.empty) {
...
async function _analyseSkillDemand(jobs) {
  ...
  for (const [skill, count] of skillCounts.entries()) {
    ...
    const ref   = db.collection(COLLECTIONS.SKILL_DEMAND).doc(docId);
    batch.set(ref, {
      ...buildSkillDemandDoc({ skill_name: skill, demand_score, growth_rate, industry_usage: industries, job_count: count }),
...
async function _analyseCareerDemand(jobs) {
  ...
  for (const [career, careerJobs] of careerGroups.entries()) {
    ...
    const ref   = db.collection(COLLECTIONS.CAREER_SCORES).doc(docId);
...
async function loadCareerScores() {
  const snap = await db.collection(COLLECTIONS.CAREER_SCORES).get();
  const result = {};
  for (const doc of snap.docs) {
    const d = doc.data();
    if (d.career_name) result[d.career_name] = d;
  }
  return result;
}
...
async function loadSkillDemand() {
  const snap = await db
    .collection(COLLECTIONS.SKILL_DEMAND)
...
module.exports = { runFullAnalysis, loadCareerScores, loadSkillDemand, TITLE_MAP };
