 ...
 *   - Cache computed results in Firestore (gcid_aggregated_cache).
 ...
 *   - edu_career_predictions (Firestore) → aggregated student outcomes
 *   - edu_education_roi (Firestore) → ROI data from student analyses
 */
...
const { COLLECTIONS, METRIC_NAMES, buildSnapshotDoc, buildCacheDoc } =
  require('../models/analyticsSnapshot.model');

// ─── In-memory cache (avoids Firestore reads on every API hit) ────────────────
...
function _memSet(key, value) { _memCache[key] = { value, ts: Date.now() }; }
...
async function getCareerDemand() {
  ...
  let liveScores = {};
  try { liveScores = await marketTrend.getCareerScoresMap(); } catch (_) {}
  ...
  const careers = Object.entries(CAREER_BENCHMARKS).map(([name, b]) => {
    ...
    const index = Math.round(
      (demand * 0.50) + (trend * 0.30) + ((100 - autoRisk) * 0.20)
    );
...
async function getSkillDemand() {
  ...
  let liveSkills = [];
  try { liveSkills = await marketTrend.getSkillDemand(30); } catch (_) {}
...
async function getEducationROI() {
  ...
  // Optionally enrich with aggregated student ROI data from Firestore
  let studentROIAvg = {};
  try {
    const snap = await db.collection('edu_education_roi').get();
    const grouped = {};
    ...
    for (const [path, scores] of Object.entries(grouped)) {
      studentROIAvg[path] = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
    }
  } catch (_) {}
...
async function _persistSnapshot(metricName, data) {
  try {
    ...
    // Write cache doc
    await db.collection(COLLECTIONS.AGGREGATED_CACHE).doc(metricName).set({
      ...buildCacheDoc(metricName, data),
      computed_at: FieldValue.serverTimestamp(),
    }, { merge: true });

    // Write snapshot doc (for historical trend lines)
    const snapRef = db.collection(COLLECTIONS.SNAPSHOTS).doc(`${metricName}_${today}`);
    const existing = await snapRef.get();
    if (!existing.exists) {
      await snapRef.set({
        ...buildSnapshotDoc({ metric_name: metricName, metric_value: data, region: 'india', snapshot_date: today }),
