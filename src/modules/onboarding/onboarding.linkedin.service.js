...
async function importLinkedIn(userId, linkedInPayload) {
  ...
  if (!linkedInPayload || typeof linkedInPayload !== 'object') {
    throw new AppError('LinkedIn profile payload must be a JSON object.', 400, {}, ErrorCodes.VALIDATION_ERROR, 'Please upload a valid LinkedIn data export. Go to LinkedIn Settings → Data Privacy → Get a copy of your data.');
  }
  ...
  const hasPositions = Array.isArray(linkedInPayload.Positions || linkedInPayload.positions || linkedInPayload.Experience || linkedInPayload.experience) && (linkedInPayload.Positions || linkedInPayload.positions || linkedInPayload.Experience || linkedInPayload.experience).length > 0;
  ...
  if (!hasEducation && !hasPositions && !hasSkills) {
    throw new AppError('LinkedIn payload does not contain recognisable Education, Positions, or Skills sections.', 422, {
      hint: 'Expected keys: Education[], Positions[], Skills[] (LinkedIn export) or education[], positions[], skills[] (API format).'
    }, ErrorCodes.VALIDATION_ERROR, 'This doesn\'t look like a LinkedIn export. Please download your LinkedIn data from Settings → Data Privacy → Get a copy of your data, then upload the JSON file.');
  }
  ...
  const rawPositions = linkedInPayload.Positions || linkedInPayload.positions || linkedInPayload.Experience || linkedInPayload.experience || [];
  const mappedExperience = rawPositions.map((p, i) => {
    ...
    return {
      ...
      industryId: null,
      // user must confirm — we don't guess the enum from raw text
      jobFunction: null,
      // same — requires user confirmation
      experienceType: 'full_time',
      // default; user can change in Step 1
      startDate,
      endDate,
      isCurrent,
      responsibilities,
      achievements: [],
      // user must add structured achievements
      _source: 'linkedin_import',
      _importIndex: i
    };
  }).filter(Boolean);
  ...
  const { error: upsertError } = await supabase
    ...
    .upsert({
      ...
      // P3-09: track whether profile was seeded from LinkedIn or manual entry
  ...
  logger.info('[OnboardingService] LinkedIn profile imported', {
    userId,
    ...importSummary
  });
  return {
...
async function confirmLinkedInImport(userId) {
  ...
  if (!progress) {
    throw new AppError('No onboarding data found. Please import your LinkedIn profile first.', 404, {
      userId
    }, ErrorCodes.NOT_FOUND, 'We couldn\'t find your imported LinkedIn data. Please try importing your profile again.');
  }

  const profileData = profile || {};

  if (!progress.importedProfile) {
    throw new AppError('No LinkedIn import found to confirm. Please import your LinkedIn profile first.', 422, {
      userId
    }, ErrorCodes.VALIDATION_ERROR, 'No LinkedIn import found. Please use the "Import from LinkedIn" button first.');
  }
  ...
  const mergedSkills = cleanSkills.length > 0 ? cleanSkills : profileData.skills || [];

  // Also extract expectedRoleIds from experience if not already set
  const existingRoleIds = profileData.expectedRoleIds || [];

  const now = new Date().toISOString();

  // Write promoted data to onboardingProgress and userProfiles in parallel
  await Promise.all([
    supabase.from('onboardingProgress').upsert({
      ...
      importSource: 'linkedin',
      // P3-09: confirm importSource
      importConfirmedAt: now,
      step: 'linkedin_confirmed',
      quickStartCompleted: true,
      // LinkedIn import counts as quick start
  ...
  const [{ data: updatedProgress }, { data: updatedProfile }] = await Promise.all([
    supabase.from('onboardingProgress').select('*').eq('id', userId).maybeSingle(),
    supabase.from('userProfiles').select('*').eq('id', userId).maybeSingle()
  ]);
