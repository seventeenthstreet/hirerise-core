...
function getEngine(name) {
  ...
  try {
    ...
    logger.warn(`[Workers] Engine not available: ${name}`, {
      err: err.message
    });
    _engines[name] = null;
  }
...
class SkillGraphWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.SKILL_GRAPH;
  }
  get concurrency() {
    return 3;
  }
  get resultTableName() {
    return 'skill_graph_results';
  } // future table — for now results go to cache
  get cacheKeyPrefix() {
    return 'worker:skill-graph';
  }
  async process(job, envelope) {
    ...
    const [graphResult, gapResult] = await Promise.allSettled([svc.getUserSkillGraph(userId), svc.detectSkillGap ? svc.detectSkillGap(userId) : Promise.resolve(null)]);
...
class CareerHealthWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.CAREER_HEALTH;
  }
  get concurrency() {
    return 2;
  }
  get resultTableName() {
    return 'career_health_results';
  }
  get cacheKeyPrefix() {
    return 'worker:career-health';
  }
  async process(job, envelope) {
    const {
      userId,
    ...
    if (!profile.target_role) {
      logger.info('[CareerHealthWorker] No target_role — skipping CHI', {
        userId
      });
      return {
        chi_score: null,
  ...
  async _loadProfile(userId) {
    ...
    const snap = await supabase.from('userProfiles').select("*").eq("id", userId).single();
...
class JobMatchingWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.JOB_MATCHING;
  }
  get concurrency() {
    return 3;
  }
  ...
  async process(job, envelope) {
    ...
    if (process.env.FEATURE_SEMANTIC_MATCHING === 'true') {
      try {
        ...
        if (semanticSvc) {
          ...
          const skillGap = skillGraphSvc ? await skillGraphSvc.getUserSkillGraph(userId).catch(() => null) : null;
          const candidateResult = await jobMatchSvc.getJobMatches(userId, {
            limit: 50
          });
          ...
          const semanticResult = await semanticSvc.getSemanticJobRecommendations(userProfile, candidates, {
            topN: 10,
            minScore: 30
          });
        ...
        logger.warn('[JobMatchingWorker] Semantic fallback to keyword', {
          err: err.message
        });
      }
    }
    ...
    if (!result) {
      result = await jobMatchSvc.getJobMatches(userId, {
        limit: 10
      });
    }
    return {
...
class RiskAnalysisWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.RISK_ANALYSIS;
  }
  get concurrency() {
    return 2;
  }
  ...
  _stubResult(userId) {
    logger.info('[RiskAnalysisWorker] Using stub result', {
      userId
    });
    return {
...
class OpportunityRadarWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.OPPORTUNITY_RADAR;
  }
  get concurrency() {
    return 2;
  }
  get resultTableName() {
    return 'opportunity_radar_results';
  }
  get cacheKeyPrefix() {
    return 'worker:opportunity-radar';
  }
  async process(job, envelope) {
    const {
      userId
...
class CareerAdvisorWorker extends BaseWorker {
  get queueName() {
    return QUEUE_NAMES.CAREER_ADVISOR;
  }
  get concurrency() {
    return 1;
  } // LLM — keep concurrency low
  get resultTableName() {
    return 'career_advice_results';
  }
  get cacheKeyPrefix() {
    return 'worker:career-advisor';
  }
  async process(job, envelope) {
    const {
      userId
    ...
    const [skillGapRes, jobMatchRes, marketRes] = await Promise.allSettled([skillGraphSvc ? skillGraphSvc.detectSkillGap ? skillGraphSvc.detectSkillGap(userId) : skillGraphSvc.getUserSkillGraph(userId) : Promise.resolve(null), jobMatchSvc ? jobMatchSvc.getJobMatches(userId, {
      limit: 5
    }) : Promise.resolve(null), marketSvc ? marketSvc.getTrendingSkills ? marketSvc.getTrendingSkills() : Promise.resolve(null) : Promise.resolve(null)]);
