...
const GenerateCoverLetterSchema = z.object({
  ...
  jobDescription: z.string().min(50, 'Job description must be at least 50 characters').max(3500, 'Job description must not exceed 3500 characters'),
  tone: z.enum(['professional', 'confident', 'conversational', 'formal']).optional().default('professional')
}).strict();
...
function requirePaidTier(req, res, next) {
  ...
  if (tier === 'free') {
    return next(new AppError('Cover letter generation is a Pro feature. Upgrade your plan to access it.', 403, {
...
async function creditGuard(req, res, next) {
  try {
    ...
    const doc = await supabase.from('users').select("*").eq("id", userId).single();
    if (!doc.exists) {
    ...
    if (available < COVER_LETTER_CREDIT_COST) {
      return next(new AppError('Insufficient AI credits. Please purchase a new plan to continue.', 402, {
...
router.post('/generate', requirePaidTier,
// 1. Block free users immediately
tierQuotaMiddleware,
// 2. Monthly cap check (no-op if tierQuota not installed)
creditGuard,
// 3. Fast credit check
validateBody(GenerateCoverLetterSchema),
// 4. Validate + sanitize body
