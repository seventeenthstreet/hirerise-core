 ...
 * Shows a read-only summary of every step before final submission.
 * "Submit for Analysis" calls the already-saved data through the orchestrator.
 */
...
function Pill({ children, color }) {
  return (
    <span style={{ ...S.pill, borderColor: color || '#1f2937', color: color || '#9ca3af' }}>
      {children}
    </span>
  );
}
...
export default function ReviewPage({ education, formData, onSubmit }) {
  const { goBack, loading, error } = education;

  const { profile, academics, activities, cognitive } = formData;
      ...
      <div className="edu-card">
        ...
        <Section title="👤 Your Profile">
          <div style={S.row}>
            ...
            <span style={S.rowValue}>{profile?.name || '—'}</span>
          </div>
          <div style={S.row}>
            <span style={S.rowLabel}>Email</span>
            <span style={S.rowValue}>{profile?.email || '—'}</span>
          </div>
          <div style={S.row}>
            <span style={S.rowLabel}>Education Level</span>
            <span style={S.rowValue}>
              {LEVEL_LABEL[profile?.education_level] || profile?.education_level || '—'}
            </span>
          </div>
        </Section>
            ...
            <table style={S.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...S.th, textAlign: 'right' }}>Marks</th>
                </tr>
              </thead>
              <tbody>
                {academics.map((r, i) => (
                  <tr key={i} style={i % 2 === 1 ? S.trAlt : {}}>
                    ...
                    <td style={S.td}>{LEVEL_LABEL[r.class_level] || r.class_level}</td>
                    <td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>
                      <span style={{ color: r.marks >= 75 ? '#22c55e' : r.marks >= 50 ? '#f59e0b' : '#ef4444' }}>
                        {r.marks}%
                      </span>
                    </td>
                  </tr>
              ...
              {activities.map((a, i) => (
                <Pill key={i}>
                  ...
                  <span style={{ opacity: 0.5, marginLeft: 6 }}>
                    {ACTIVITY_LEVEL_LABEL[a.activity_level] || a.activity_level}
                  </span>
                </Pill>
              ))}
            </div>
          ...
          {cognitive ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {COGNITIVE_KEYS.map(({ key, label }) => {
                const val = cognitive[key] ?? 0;
                const { text, color } = COGNITIVE_LABEL(val);
                return (
                  <div key={key} style={S.cogRow}>
                    <span style={S.cogLabel}>{label}</span>
                    <div style={S.cogBarWrap}>
                      <div style={{ ...S.cogBar, width: `${val}%`, background: color }} />
                    </div>
                    <span style={{ ...S.cogScore, color }}>{val}</span>
                    <span style={{ ...S.cogText, color }}>{text}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
          <button className="edu-btn edu-btn-secondary" onClick={goBack} disabled={loading}>
            ← Back
          </button>
          <button
        ...
        <p style={S.disclaimer}>
          Your data is saved securely. Stream analysis results will appear on your dashboard
...
const LOCAL_STYLES = `
  @media (max-width: 600px) {
    .review-cog-row { grid-template-columns: 1fr 80px 28px 60px !important; gap: 6px !important; }
  }
`;
