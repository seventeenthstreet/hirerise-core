...
} = require('../agents/riskAndRadarAgents');
...
async function _loadUserProfile(userId) {
  try {
    ...
    logger.warn('[Coordinator] Profile load failed', {
      userId,
      err: err.message
    });
    return {
...
async function coordinate(userId, userQuery = null, opts = {}) {
  ...
  if (!forceRefresh) {
    try {
      ...
      if (hit) {
        ...
        logger.debug('[Coordinator] Cache hit', {
          userId,
          intent
        });
        return {
  ...
  const selectedAgents = agentKeys.filter(k => AGENTS[k]).map(k => ({
    key: k,
    agent: AGENTS[k]
  }));
  const settled = await Promise.allSettled(selectedAgents.map(({
    agent
  }) => agent.execute(userId, agentContext, {
    forceRefresh
  })));
  ...
  selectedAgents.forEach(({
    ...
    if (result.status === 'fulfilled' && result.value.output !== null) {
      ...
      agentErrors.push({
        agent: selectedAgents[i].agent.agentName,
        error: errMsg
      });
  ...
  logger.info('[Coordinator] Agents done', {
    userId,
    succeeded: agentsUsed.length,
    failed: agentErrors.length,
    cached: cachedCount
  });

  // ── CareerAdvisorAgent: synthesis ─────────────────────────────────────────
  const advisorResult = await advisorAgent.execute(userId, {
    userQuery,
    agentOutputs,
    userProfile
  }, {
  ...
  const confidence = Math.round(Math.min(0.99, dataCompleteness * 0.6 + (advisorOutput ? 0.4 : 0.1)) * 1000) / 1000;
  ...
  Promise.all([_upsertSession(userId, sessionId, intent, agentsUsed), _persistResponse(userId, sessionId, turnIndex, {
    userQuery,
    intent,
    agentsUsed,
    agentErrors,
...
async function getAgentStatus() {
  return {
    agents: Object.fromEntries([...Object.entries(AGENTS), ['advisor', advisorAgent]].map(([k, a]) => [k, {
      name: a.agentName,
...
async function invalidateUserCache(userId) {
  const tasks = [
  // Per-agent caches
  ...Object.values(AGENTS).map(a => a.invalidateCache(userId)), advisorAgent.invalidateCache(userId),
  // Coordinator caches (all intents)
  ...Object.keys(INTENT_MAP).map(intent => cache.del(_cacheKey(userId, intent)).catch(() => {}))];
  await Promise.allSettled(tasks);
  logger.info('[Coordinator] Cache invalidated', {
    userId
  });
}
