...
const COPILOT_CONVERSATIONS_COLLECTION = 'copilot_conversations_firestore';
// Note: also persisted to Supabase copilot_conversations table
...
async function _loadHistory(userId, conversationId) {
  // Try Supabase first (more structured, joinable with context)
  try {
  ...
  try {
    const snap = await db
      .collection(COPILOT_CONVERSATIONS_COLLECTION)
...
async function _saveConversationTurn(userId, conversationId, turnIndex, {
  userMessage, aiResponse, dataSources, confidence, ragContextId,
}) {
  const turn = {
    user_id:         userId,
  ...
  // Write to Supabase
  supabase.from('copilot_conversations').insert(turn).then(() => {}).catch(() => {});

  // Also write to Firestore for backward compat with existing history endpoints
  db.collection(COPILOT_CONVERSATIONS_COLLECTION)
    .add({ ...turn, created_at: FieldValue.serverTimestamp() })
    .catch(() => {});
}

async function _saveRAGContext(userId, conversationId, turnIndex, {
  userQuery, ragContext, contextObj, aiResponse, confidence, refused, refusalReason,
}) {
  const { data } = await supabase
...
async function chat(userId, userMessage, opts = {}) {
  ...
  logger.info('[CopilotService] Chat request', {
    userId, messageLen: userMessage.length, conversationId,
  });
  ...
  try {
    ...
    logger.error('[CopilotService] Context retrieval failed', { userId, err: err.message });
    ragContext = {
      data_sources_used: [],
      data_completeness: 0,
      confidence_score:  0,
  ...
  if (!preCheck.allowed) {
    logger.info('[CopilotService] Pre-flight refused', {
      userId, intent: preCheck.intent,
    });

    // Log grounding failure
    supabase.from('copilot_grounding_failures').insert({
      user_id:           userId,
      user_query:        userMessage,
      missing_sources:   Object.keys(ragContext).filter(k =>
        !['data_sources_used','data_completeness','confidence_score',
          'is_sufficient','retrieval_ms','retrieved_at','_cached'].includes(k) &&
        ragContext[k] === null
      ),
      data_completeness: ragContext.data_completeness,
  ...
  const { cleanedResponse, violations, wasModified } =
    groundingGuard.postFlightScan(rawResponse, ragContext);

  if (violations.length > 0) {
    logger.warn('[CopilotService] Hallucination patterns detected', {
      userId, violations, wasModified,
    });
  }

  // ── Step 7: CALCULATE CONFIDENCE ─────────────────────────────────────────
  const confidence = groundingGuard.calculateResponseConfidence(
    ragContext,
    preCheck.intent,
    contextObj.dataSources
  );
  ...
  try {
    ...
    logger.warn('[CopilotService] Failed to persist conversation', { userId, err: err.message });
  }

  const durationMs = Date.now() - startMs;
  logger.info('[CopilotService] Response generated', {
    userId, confidence, sources: contextObj.dataSources.length,
    violations: violations.length, durationMs,
  });
...
async function getHistory(userId, conversationId) {
  ...
  return { conversations: data || [], conversation_id: conversationId };
}
...
async function getWelcome(userId) {
  ...
  const dataNote = dataParts.length > 0
...
async function _loadUserName(userId) {
  try {
    ...
    return snap.exists ? (snap.data().name || snap.data().fullName || null) : null;
  } catch (_) { return null; }
}
...
module.exports = { chat, getHistory, getWelcome };
