 ...
 * Credentials are retrieved ONLY from Secret Manager — never from Firestore
 ...
 *
 * Output signals stored in Firestore collection: role_market_demand
...
async function _getProviderConfig() {
  ...
  throw Object.assign(
    new Error(`Unknown market API provider: ${provider}`),
    { statusCode: 400, code: 'INVALID_PROVIDER' }
  );
}
...
async function _fetchFromCustomAPI(config, role, country) {
  ...
  const data  = await res.json();
  const count = data.count ?? data.total ?? data.results?.length ?? data.jobs?.length ?? 0;
 ...
 * calls the configured API, and stores results in Firestore.
 *
...
async function fetchMarketDemand(role, country = 'in') {
  ...
  logger.info('[MarketDemand] Fetching demand signals', {
    provider: config.provider, role, country,
  });
  ...
  // Store in Firestore — processed signals only, never credentials
  ...
  logger.info('[MarketDemand] Signals stored', {
    docId, job_postings: record.job_postings, provider: config.provider,
  });
...
async function getProviderStatus() {
  try {
    ...
    // Find last sync from Firestore
    const snap = await db.collection(COLLECTION)
      .orderBy('fetchedAt', 'desc')
      .limit(1)
      .get();

    const lastSync = snap.empty ? null : snap.docs[0].data().fetchedAt;
    ...
    return { provider: null, isConfigured: false, lastSync: null };
  }
}
