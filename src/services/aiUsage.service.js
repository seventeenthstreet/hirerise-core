  ...
  try {
    await db.runTransaction(async (txn) => {
      ...
      if (currentCount >= limit) {
        ...
        throw new AppError(
          `Monthly AI usage limit reached for your ${tier} plan. Resets on ${resetAt.slice(0, 10)}.`,
          429,
          {
            monthlyLimit:  limit,
            currentCount,
      ...
      const newReset = needsReset
        ? Timestamp.fromDate(nextResetDate(now))
        : (snap.exists ? snap.data().aiUsageResetDate : Timestamp.fromDate(nextResetDate(now)));
