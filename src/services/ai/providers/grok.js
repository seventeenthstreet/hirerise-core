...
/**
 * @file src/services/ai/providers/grok.js
 * @description
 * xAI Grok resume extraction provider (priority 2).
 *
 * Uses xAI's OpenAI-compatible REST endpoint:
 *   https://api.x.ai/v1/chat/completions
 *
 * No SDK required — pure fetch with:
...
async function resolveApiKey() {
  ...
  // Env var takes priority — note: env uses GROK_API_KEY per project convention
  ...
  if (typeof getSecret === 'function') {
    for (const name of ['GROK_API_KEY', 'XAI_API_KEY']) {
      const secret = await getSecret(name).catch(() => null);
...
/**
 * Extract resume fields using xAI Grok.
 *
...
async function extractResume(resumeText) {
  ...
  try {
    ...
    if (!apiKey) throw new Error('[Grok] GROK_API_KEY / XAI_API_KEY is not configured');
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `[Grok] HTTP ${response.status}: ${errBody.slice(0, 200)}`
      );
    }
    ...
    if (!rawText) throw new Error('[Grok] Empty response body');

    const parsed = safeParseJson(rawText);
    if (!parsed) throw new Error('[Grok] Failed to parse JSON response');

    logger.info('[AIProviderManager] Grok extraction success', {
    ...
    logger.error('[AIProviderManager] Grok extraction failed', {
      provider:   PROVIDER_NAME,
      latency_ms: Date.now() - startedAt,
      error:      err.message,
    });
