...
const log = {
  info: (message: string) => console.log(`[EmbeddingBackfill] ${message}`),
  warn: (message: string) => console.warn(`[EmbeddingBackfill] ${message}`),
...
async function embedWithRetry(role: RoleRow): Promise<number[]> {
  ...
  throw new Error(`Embedding failed for role ${role.role_name}`);
}

async function updateEmbedding(roleId: string, embedding: number[]): Promise<void> {
  ...
  if (error) {
    throw new Error(`Supabase update failed for role ${roleId}: ${error.message}`);
  }
}

async function processRole(role: RoleRow): Promise<string> {
...
async function backfillEmbeddings(): Promise<void> {
  ...
  log.info(`Starting backfill for ${roles.length} roles...`);
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    const results = await Promise.allSettled(batch.map(processRole));

    for (const result of results) {
      if (result.status === 'fulfilled') {
        successCount++;
        log.info(`Backfilled ${successCount}/${roles.length} — ${result.value}`);
      } else {
        log.error('Role backfill failed', result.reason);
        throw result.reason;
      }
    }
  ...
  log.info(`Backfill complete. Roles updated: ${successCount}`);
}
