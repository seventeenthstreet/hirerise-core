...
-- Verification: migration_M3_security_foundation_VERIFY_v1.sql
--
...
-- Verification Status:   VERIFICATION ARTIFACT — for execution in staging
--                        after migration_M3_security_foundation_v1.sql deployment
--
...
SELECT
  ...
  CASE
    WHEN tablename = 'signal_lineage' AND policyname = 'lineage_service_full'
      THEN CASE WHEN roles @> ARRAY['service_role'::name] AND cmd = 'ALL'
                THEN 'PASS' ELSE 'FAIL — wrong role or cmd' END
    WHEN tablename = 'signal_lineage' AND policyname = 'lineage_audit_read'
      THEN CASE WHEN roles @> ARRAY['governance_audit'::name] AND cmd = 'SELECT'
                THEN 'PASS' ELSE 'FAIL — wrong role or cmd' END
    ...
    WHEN tablename = 'signal_category_hierarchy' AND policyname = 'hierarchy_read_all'
      THEN CASE WHEN roles @> ARRAY['anon'::name, 'authenticated'::name] AND cmd = 'SELECT'
...
SELECT
  ...
  CASE
    WHEN qual IS NULL AND cmd = 'ALL'    THEN 'INVESTIGATE — ALL policy has no USING clause'
    WHEN qual = '(true)'                 THEN 'PASS — USING (true)'
    WHEN qual IS NULL AND cmd = 'SELECT' THEN 'INVESTIGATE — SELECT policy has no USING clause'
...
-- migration_M3_security_foundation_VERIFY_v1.sql
-- Sprint 1B / Package G3 / Gate G3
-- Generated: 2026-06-09 per Phase 2A.1.3 A01.F06 Step 2
--
