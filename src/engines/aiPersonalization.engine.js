...
function _getSkillGraphSvc() {
  if (!_skillGraphSvc) {
    try {
      _skillGraphSvc = require('../modules/jobSeeker/skillGraphEngine.service');
    } catch (_) {}
  }
...
function _getJobMatchSvc() {
  if (!_jobMatchSvc) {
    try {
      _jobMatchSvc = require('../modules/jobSeeker/jobMatchingEngine.service');
    } catch (_) {}
  }
  return _jobMatchSvc;
}
...
async function updateBehaviorProfile(userId) {
  ...
  const {
    data: events,
    error
  } = await supabase.from('user_behavior_events').select('event_type, entity_type, entity_id, entity_label, timestamp, metadata').eq('user_id', userId).gte('timestamp', windowStart.toISOString()).order('timestamp', {
    ascending: false
  ...
  if (!events || events.length === 0) {
    logger.info('[PersonalizationEngine] No events found — creating empty profile', {
      userId
    });
  ...
  for (const event of events) {
    ...
    if (event.entity_type === 'role' && event.entity_id) {
      const existing = roleSignals.get(label) || {
        score: 0,
        click_count: 0,
        apply_count: 0
      };
    ...
    if (event.entity_type === 'skill' && event.entity_id) {
      const existing = skillSignals.get(label) || {
        score: 0,
        view_count: 0
      };
      existing.score += eventScore;
  ...
  const preferredRoles = [...roleSignals.entries()].map(([name, data]) => ({
    name,
    score: Math.round(data.score / maxRoleScore * 100) / 100,
    click_count: data.click_count,
    apply_count: data.apply_count
  })).sort((a, b) => b.score - a.score).slice(0, 10);
  const preferredSkills = [...skillSignals.entries()].map(([name, data]) => ({
    name,
    score: Math.round(data.score / maxSkillScore * 100) / 100,
    view_count: data.view_count
  })).sort((a, b) => b.score - a.score).slice(0, 15);
  const careerInterests = [...industryMap.entries()].map(([industry, score]) => ({
    industry,
...
async function _upsertProfile(userId, profile) {
  const {
    error
  } = await supabase.from('user_personalization_profile').upsert({
    user_id: userId,
    ...profile
  }, {
    onConflict: 'user_id'
  ...
  try {
    await cache.del(`personalization:recommendations:${userId}`);
  } catch (_) {}
  return {
    user_id: userId,
    ...profile
...
async function recommendPersonalizedCareers(userId, opts = {}) {
  ...
  if (!forceRefresh) {
    try {
      ...
      if (cached) {
        logger.debug('[PersonalizationEngine] Cache hit', {
          userId
        });
        return {
          ...JSON.parse(cached),
  ...
  const {
    data: profileRow
  } = await supabase.from('user_personalization_profile').select('*').eq('user_id', userId).maybeSingle();
  ...
  const [firestoreProfile, jobMatchResult, radarResult, skillGapResult, marketResult] = await Promise.allSettled([_loadFirestoreProfile(userId), _getJobMatchSvc() ? _getJobMatchSvc().getJobMatches(userId, {
    limit: 30
  }) : Promise.resolve(null), _loadOpportunitySignals(), _getSkillGraphSvc() ? _getSkillGraphSvc().getUserSkillGraph(userId) : Promise.resolve(null), _getMarketSvc() ? _getMarketSvc().getSkillDemand(30) : Promise.resolve([])]);
  const userProfile = firestoreProfile.status === 'fulfilled' ? firestoreProfile.value : {
    skills: []
  };
  ...
  const marketDemandMap = new Map((marketDemand || []).map(s => [(s.skill_name || s.name || '').toLowerCase(), s.demand_score || 0]));
  ...
  const behaviorRoles = new Map((profile.preferred_roles || []).map(r => [r.name.toLowerCase(), r.score]));
  const behaviorSkills = new Map((profile.preferred_skills || []).map(s => [s.name.toLowerCase(), s.score]));
  const scored = [];
  for (const [roleName, candidate] of candidateMap) {
    // Component A: Behavior signal (0–100)
    const behaviorScore = Math.round((behaviorRoles.get(roleName.toLowerCase()) || 0) * 100);
    ...
    const behaviorSkillBoost = [...behaviorSkills.entries()].filter(([skill]) => _isRelevantToRole(skill, roleName)).reduce((sum, [, score]) => sum + score * 20, 0); // each relevant behavior skill adds up to 20 pts
    ...
    const rawScore = P_WEIGHTS.behavior_signals * behaviorScore + P_WEIGHTS.skill_alignment * skillAlignScore + P_WEIGHTS.opportunity_score * opportunityScore + P_WEIGHTS.market_demand * marketScore;

    // If no personalization signal, fall back to base match score
    const finalScore = hasSignal ? Math.round(Math.min(99, rawScore)) : Math.round(Math.min(99, existingMatchPct * 0.6 + opportunityScore * 0.4));
    if (finalScore < 20) continue; // filter noise

    // Build match reason string
    const matchReason = _buildMatchReason(behaviorScore, skillAlignScore, opportunityScore, profile, roleName);
    scored.push({
      role: roleName,
      score: finalScore / 100,
      // normalise 0–99 → 0–1 to match frontend Math.round(r.score*100) display
  ...
  const topRoles = scored.sort((a, b) => b.score - a.score).slice(0, topN);
  const result = {
    user_id: userId,
...
async function trackBehaviorEvent(userId, eventData) {
  ...
  try {
    const {
      count
    } = await supabase.from('user_behavior_events').select('id', {
      count: 'exact',
      head: true
    }).eq('user_id', userId);
  ...
  return {
    id: data.id,
    queued_profile_update: queuedProfileUpdate
  };
}
...
async function applyPersonalizationBoost(userId, items, itemType = 'job') {
  ...
  const roleSignals = new Map((profile.preferred_roles || []).map(r => [r.name.toLowerCase(), r.score]));
  const skillSignals = new Map((profile.preferred_skills || []).map(s => [s.name.toLowerCase(), s.score]));
  return items.map(item => {
    const name = (item.title || item.role || item.name || '').toLowerCase();
    const roleBoost = (roleSignals.get(name) || 0) * 15; // up to 15 point boost
    const skillBoost = (item.missing_skills || item.skills || []).reduce((sum, skill) => sum + (skillSignals.get(skill.toLowerCase()) || 0) * 5, 0);
    return {
      ...item,
...
async function _loadOpportunitySignals() {
  const {
    data
  } = await supabase.from('career_opportunity_signals').select('role_name, industry, opportunity_score, demand_score, average_salary, required_skills').gte('opportunity_score', 40).order('opportunity_score', {
    ascending: false
  }).limit(30);
  return data || [];
}
...
function _buildMatchReason(behaviorScore, skillScore, opportunityScore, profile, roleName) {
  const parts = [];
  if (behaviorScore > 50) parts.push(`frequently viewed ${roleName} roles`);else if (behaviorScore > 20) parts.push(`showed interest in ${roleName}`);
