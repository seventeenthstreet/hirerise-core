...
const CEILINGS = Object.freeze({
  growth_rate:       100,  // 100% YoY = absolute max
  salary_growth:      60,  // 60% YoY salary growth = extreme
  skill_demand:      100,  // demand_score already 0–100
...
function _growthTrendLabel(score) {
  for (const { min, label } of TREND_LABELS) {
...
function _calculateOpportunityScore(signals) {
  ...
  const score = Math.round(
    OPP_WEIGHTS.job_growth      * jobGrowth      +
    OPP_WEIGHTS.salary_growth   * salaryGrowth   +
    OPP_WEIGHTS.skill_demand    * skillDemand    +
    OPP_WEIGHTS.industry_growth * industryGrowth
  );
...
function _calculateMatchScore(userSkills, requiredSkills, adjacentSkills = []) {
  if (!requiredSkills || requiredSkills.length === 0) {
    return { match_score: 50, overlap: userSkills.slice(0, 3), to_learn: [] };
  }
  ...
  const rawScore =
    (directMatches + adjacentMatches * 0.5) / requiredSkills.length * 100;
...
async function detectOpportunitySignals() {
  ...
  if (!marketSvc) {
    ...
    return { upserted: 0, total: 0, duration_ms: Date.now() - startMs };
  }

  // Load LMI data in parallel
  const [careerTrendsRes, skillDemandRes] = await Promise.allSettled([
    marketSvc.getCareerTrends(),
    marketSvc.getSkillDemand(50),
  ]);
  ...
  const skillDemandMap = new Map(
    skillDemand.map(s => [
      (s.skill_name || s.name || '').toLowerCase(),
      s.demand_score || s.score || 0,
    ])
  );
  ...
  for (const trend of careerTrends) {
    ...
    const demandScores  = roleSkills
      .map(s => skillDemandMap.get(s.toLowerCase()) || 0)
      .filter(v => v > 0);
    const avgDemandScore = demandScores.length > 0
      ? Math.round(demandScores.reduce((a, b) => a + b, 0) / demandScores.length)
      : trend.demand_score || 50;

    const { score, breakdown, trend: trendLabel } = _calculateOpportunityScore({
    ...
    const avgSalary    = avgSalaryRaw >= 100000
      ? `₹${Math.round(avgSalaryRaw / 100000)}L`
      : avgSalaryRaw > 0 ? `₹${avgSalaryRaw.toLocaleString('en-IN')}` : 'Market Rate';
  ...
  for (let i = 0; i < signals.length; i += BATCH) {
    ...
    const { error } = await supabase
      ...
      .upsert(chunk, { onConflict: 'role_name,industry' });

    if (error) {
      logger.error('[OpportunityRadar] upsert error', { error: error.message });
    } else {
  ...
  logger.info('[OpportunityRadar] detectOpportunitySignals complete', {
    upserted, total: signals.length, durationMs,
  });

  return { upserted, total: signals.length, duration_ms: durationMs };
}
...
async function getOpportunityRadar(userId, opts = {}) {
  ...
  return _cached(cacheKey, CACHE_TTL_SECONDS, async () => {
    ...
    try {
      ...
      logger.warn('[OpportunityRadar] profile load failed', { userId, err: err.message });
      userProfile = { skills: [], targetRole: null, industry: null, yearsExperience: 0 };
    }
    ...
    for (const signal of signals) {
      const requiredSkills = Array.isArray(signal.required_skills)
        ? signal.required_skills
        : (typeof signal.required_skills === 'string'
          ? JSON.parse(signal.required_skills)
          : []);

      const { match_score, overlap, to_learn } = _calculateMatchScore(
        userProfile.skills,
        requiredSkills,
        adjacentSkills
      );
      ...
      }, { onConflict: 'user_id,signal_id' }).then(() => {}).catch(() => {});
    ...
    const topOpportunities = scored
      ...
      .slice(0, topN)
      .map(({ _rank, ...rest }) => rest); // strip internal _rank field

    logger.info('[OpportunityRadar] getOpportunityRadar', {
      userId, evaluated: signals.length, returned: topOpportunities.length,
    });
...
async function getEmergingRoles(opts = {}) {
  ...
  return _cached(cacheKey, CACHE_TTL_SECONDS, async () => {
    ...
    if (error) {
      logger.error('[OpportunityRadar] getEmergingRoles error', { error: error.message });
    ...
    const roles = (data || []).map(signal => ({
      ...
      is_emerging:       signal.is_emerging,
      required_skills:   Array.isArray(signal.required_skills)
        ? signal.required_skills
        : JSON.parse(signal.required_skills || '[]'),
      score_breakdown:   signal.score_breakdown,
    }));
...
async function _getSignals(minScore = 0) {
  ...
  return _cached(cacheKey, CACHE_TTL_SECONDS, async () => {
    ...
    if (error) {
      logger.error('[OpportunityRadar] _getSignals error', { error: error.message });
      return [];
    }
