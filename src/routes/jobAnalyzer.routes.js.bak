...
const { analyzeJobFit, getJobAnalysisHistory } = require('../services/jobAnalyzer.service');
...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      ...
      return res.status(200).json({ success: true, data: { analysis: result } });
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const snap = await db.collection('job_analyses').doc(req.params.id).get();

    if (!snap.exists || snap.data()?.userId !== userId) {
      return res.status(404).json({ success: false, message: 'Analysis not found' });
    }

    return res.status(200).json({ success: true, data: { analysis: snap.data() } });
  } catch (err) { return next(err); }
});
