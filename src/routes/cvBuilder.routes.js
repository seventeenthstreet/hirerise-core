...
const { generateJobSpecificCv, getCvVersions } = require('../services/cvBuilder.service');
...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      ...
      const result = await generateJobSpecificCv(userId, { jobDescription, jobTitle });

      return res.status(200).json({ success: true, data: { cvVersion: result } });
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    const snap = await db.collection('user_cvs').doc(req.params.id).get();

    if (!snap.exists || snap.data()?.userId !== userId) {
      return res.status(404).json({ success: false, message: 'CV version not found' });
    }

    return res.status(200).json({ success: true, data: { cvVersion: snap.data() } });
  } catch (err) { return next(err); }
});
