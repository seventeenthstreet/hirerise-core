...
function computeModelBreakdown(rows) {
  ...
  return Object.entries(models).map(([model, d]) => ({
    model,
...
class AdminMetricsService {
  ...
  async getAggregatedMetrics(params) {
    ...
    // TODO: MANUAL MIGRATION REQUIRED — sub-collection detected
    const snap = await db.collection('metrics').doc('daily').collection('snapshots').where('date', '>=', startStr).where('date', '<=', endStr).orderBy('date', 'asc').get();
    if (snap.empty) {
      return {
        period: period.label,
        startDate: startStr,
    ...
    const grossMarginUSD = parseFloat((totalRevenueUSD - totalCostUSD).toFixed(6));
    const grossMarginPercent = totalRevenueUSD > 0 ? parseFloat((grossMarginUSD / totalRevenueUSD * 100).toFixed(2)) : 0;
    const topFeatures = Object.entries(featureCounts).map(([feature, count]) => ({
      feature,
      count
    })).sort((a, b) => b.count - a.count).slice(0, 10);
    return {
